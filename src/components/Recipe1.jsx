...
const Recipe1 = () => {
    ...
    const DEFAULT_IMAGE_URL = 'default_image_url.jpg'; // Placeholder image
    ...
    // Fetch recipe by name and navigate to its ID
    const handleRecipeLinkClick = async (recipeName) => {
        try {
            const response = await fetch(
                `http://localhost:5000/api/recipe?name=${encodeURIComponent(recipeName)}`
            );
            const data = await response.json();
            ...
            if (response.ok && data.data.length > 0) {
                ...
                if (matchedRecipe) {
                    navigate(`/recipe/${matchedRecipe._id}`);
                } else {
    ...
    const recipeLinks = recipe.RecipeLinks
        ? recipe.RecipeLinks.split(',').map((link) => link.trim()).filter((link) => link)
        : [];
            ...
            <div className="recipe-detail-grid">
                <div className="recipe-detail-image">
                    <div className="image-container">
                        <img
                            src={recipe.Images || DEFAULT_IMAGE_URL}
                    ...
                    {recipe.ImagesSource && (
                        <div className="image-source">
                            ...
                            <a href={recipe.ImagesSource} target="_blank" rel="noopener noreferrer">
                                {recipe.ImagesSource}
                            </a>
                        </div>
                    )}
